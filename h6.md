...
MariaDB-server ja -client asentuivat oikein. Tein mariadb -kansioon "createuser.sql" ja lisäsin sinne sql-komennot, joilla MariaDB käyttäjä ja tietokanta saadaan luotua

ja tein tiedoston nimeltä "createuser.sql", lisäsin sinne
...
Php ja sen kirjasto asentuivat oikein. Sitten lähdin automatisoimaan käyttäjienkotisivujen php-käyttöoikeuksia:

![php7.0.conf]()

php7.0.conf -tiedosto päivittyi oikeille asetuksille. php7.0.conf -tiedosto päivittyi. <ifModule... oli kommentoitu ulos kun kävin katsomassa tilan ajon jälkeen.
